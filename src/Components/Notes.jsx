...
const Notes = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      const response = await axios.get(`${BackendURL}/contents`);
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading("post");
    try {
      await axios.post(`${BackendURL}/contents`, payload);
  ...
  return (
    <div>
      <div className="w-full flex gap-4">
        <div className="w-[30%]">
          <label className="block mb-1 text-sm text-gray-700 text-start">
            Topic Name
          </label>
          <input
            type="text"
      ...
      <div className="w-full border border-gray-300 rounded max-h-96 overflow-y-auto">
        <div className="sticky top-0 flex font-semibold bg-gray-100 border-b border-gray-300">
          ...
          <div className="w-[750px] border-r border-gray-300 p-1">
            Topic Explanation
          </div>
        ...
        <div className="">
          {[...notesList]?.reverse().map((note, index) => (
            <div key={index} className="flex border-b border-gray-300">
              <div className="w-16 border-r border-gray-300 p-1">
                {index + 1}
              </div>
              <div className="w-72 border-r border-gray-300 p-1">
                {note.topic}
              </div>
              <div className="w-[750px] border-r border-gray-300 p-1">
                {note.explanation || ""}
